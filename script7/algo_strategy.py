...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def build_defences(self, game_state):
        ...
        begin_locations = possible_locations
            ...
            if (len(self.prev_all_locations)>0):
                if (len(possible_locations) > len(self.prev_all_locations)):
                    ...
                    if statistics.stdev(destroyed_locations_x) < 4:
                        ...
                        possible_locations.remove(destroyed_locations_x[0])
                    else:
                        for x in destroyed_locations:
                            game_state.attempt_spawn(DESTRUCTOR, x)
                            possible_locations.remove(x)



        for x in possible_locations:
            attackers = game_state.get_attackers(x, 1)
            for y in attackers:
                endp = game_state.get_target(y)
                if endp in possible_locations:
                    game_state.attempt_spawn(DESTRUCTOR, endp)
                    possible_locations.remove(endp)
        ...
        self.prev_all_locations = begin_locations
    ...
    def deploy_attackers(self, game_state):
        ...
        enemy_locations = [[]]
        for i in range(game_state.ARENA_SIZE):
            for j in range(math.floor(game_state.ARENA_SIZE / 2) + 1, math.floor(game_state.ARENA_SIZE)):
                if (game_state.game_map.in_arena_bounds([i, j])):
                    enemy_locations.append([i, j])
        if (len(enemy_locations)>0):
            for i in enemy_locations:
                if (game_state.contains_stationary_unit(i)):
                    the_path = game_state.shortest_path_finder.navigate_multiple_endpoints(i, deploy_locations, self)
                    game_state.attempt_spawn(PING, the_path[the_path - 1])
                    possible_locations.remove(the_path[the_path - 1])
        '''
        ...
        while bits_to_spend >= 1 and len(deploy_locations) > 0:
            ...
            if ((self.prev_enemy_bits - enemy_bits) > 4):
